Feature: Services

Scenario:  Validate user is able to click on  Bangalore and navigate to the Interior Designers in Bangalore

Given Browser should be launched and url should be navigated
And user should be able to logged in
When user should be able to mouse hover on homeInterior link
And user should be able to click on Home Interior Design Services
And user should be able to navigated to  Home Interior Design Services page
And user should be able to click on bangalore
And user should be navigated to the Interior Designers in Bangalore 
Then user should be navigated to the Interior Designers in Bangalore